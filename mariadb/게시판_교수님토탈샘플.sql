...
CREATE TABLE `t_board_board`(
    ...
    `title` varchar(1000) NOT NULL DEFAULT 'Any',
    `writer` varchar(1000) NOT NULL DEFAULT 'Anony',
    `content` varchar(4000) DEFAULT 'No Content',
...
CREATE TABLE `t_comment_board`(
    ...
    `bno` int NOT NULL,
    PRIMARY KEY (`no`),
    CONSTRAINT `t_comment_bno_fk` FOREIGN KEY (`bno`) REFERENCES `t_board_board` (`no`)  
);
...
SELECT * FROM t_board_board;t_book_board
